...
import static org.usagreencardlottery.main.ScrapperWithShutdownHook.ScrapLog;
import static org.usagreencardlottery.main.ScrapperWithShutdownHook.isInterrupted;
import static org.usagreencardlottery.main.ScrapperWithShutdownHook.processingApplication;
...
public class ScrapperWithShutdownHook {
    ...
    public static AtomicBoolean processingApplication = new AtomicBoolean(true);
    ...
    public static void main(String[] args) {
        ...
        runtime.addShutdownHook(new Thread() {
            ...
            public void run() {
                ...
                try {
                    while (processingApplication.get()) {
                        ScrapLog.info("PLEASE WAIT WHILE THE CURRENT APPLICATION IS SUBMITTED SUCCESSFULLY...");
                        Thread.sleep(3000);
                    }
                } catch (InterruptedException ex) {
                    java.util.logging.Logger.getLogger(ScrapperWithShutdownHook.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
        });
        ...
        int counter = 0;
        String action = "";
        String skipList = "/1564610/362672/439723/1699923/439723/405450/1208616/1545554/154950/370183/441651/875884/1078982/813526/4384456/4420534/1932195/2393031/4109006/3826430/2590281/3925858/2754709/2816693/";
        int userId = 0;
        int applicationId = 0;
        ...
        connector.loadConfig("/home/mageshwaran/NetBeansProjects/DVLotterySubmission/src/gclocal.properties");
        try {
            ...
            rs = stmt.executeQuery("select Registration.userId, "
                    ...
                    + "from Application, Registration, EligibilityHistory "
                    + "where Application.applicationStatus = 10 and "
                    + "EligibilityHistory.endYear >= " + currentYear + " and "
                    + "(Application.post2009 is NULL or Application.post2009 != 12) and "
            ...
            while (rs.next() && !isInterrupted.get()) {
                processingApplication.set(true);
                counter++;
                userId = rs.getInt(1);
                if ((!skipList.contains(Integer.toString(userId))) && (rs.getString(9) != null) && (!"null".equals(rs.getString(9))) && (!rs.getString(9).equals(""))) {
                    ...
                    if (endYear >= currentYear && winner == 0 && isPaymentConnected(applicationId)) {
                    ...
                    if (comments.equals("Need to service")) {
                        ...
                        if (applicationStatus == 10) {
                            ...
                            boolean isSpouseIncluded = thisApp.isCouplePlan() && thisApp.getApplicant().getMaritalStatus().equals("2");
                            if (isSpouseIncluded) {
                            ...
                            boolean submitPrimary = (getSubmissionStatus(applicationId) < 11);
                            boolean submitSpouse = isSpouseIncluded && getSubmissionStatus(applicationId) != 12;
                            needSubmission = (submitPrimary || submitSpouse);
                            if (thisApp.getApplicant().getLivingCountry() == null || thisApp.getApplicant().getLivingCountry().equals("null") || thisApp.getApplicant().getLivingCountry().equals("")) {
                                needSubmission = false;
                                action = "[CRITICAL] Living Country is empty - So not considering";
                            }
                            if (!needSubmission) {
                                action = "SUBMISSION NOT REQUIRED";
                                if (getSubmissionStatus(applicationId) > 10) {
                                ...
                                if (submitPrimary) {
                                    ...
                                    if (thisPost.submitApplication("P")) {
                                        ...
                                        updatePost(thisApp.getUserId(), 11);
                                ...
                                if (submitSpouse) {
                                    if (thisApp.getSpouse().getEducation().equals("0") || thisApp.getSpouse().getEducation().equals("null")) {
                                        ...
                                        if (thisPost.submitApplication("S")) {
                                            ...
                                            updatePost(thisApp.getUserId(), 12);
                ...
                processingApplication.set(false);
            }
            ScrapLog.info("EITHER THE APPLICATION WAS ALREADY SUBMITTED | OR | YOU WANTED TO QUIT FOR A BREAK");
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
    ...
    public static AppDetails getAppDetails(int userId, int appId, String nativeMode, String nativeCountry) {
        ...
        String couplePlans = "-4-5-6-12-13-14-19-20-21-25-26-27-30-31-32-41-42-43-79-80-81-89-90-91-7-8-15-28-29-37-44-65-66-67-68-69-70-74-78-82-84-88-92-95-98-101-104-";
        String plan = "1";
        ...
        try {
            ...
            if (rs.next()) {
                ...
                if (thisApp.getApplicant().getBirthCity().trim().equals("") || thisApp.getApplicant().getBirthCity().trim().equals("null")) {
        ...
        if (thisApp.getApplicant().getMaritalStatus().equals("2")) {
    ...
    public static int plans(int pId) {
        String one = "/1/4/9/12/16/19/22/25/32/35/36/38/41/47/49/51/52/53/54/55/71/72/73/75/79/83/85/89/93/96/99/102/105/";
        String two = "/2/5/10/13/17/20/23/26/31/34/39/42/45/48/50/56/57/58/59/76/80/86/90/94/97/100/103/";
        String four = "/3/6/11/14/18/21/24/27/30/33/40/43/46/60/61/62/63/64/77/81/87/91/";
        String multi = "/7/8/15/28/29/37/44/65/66/67/68/69/70/74/78/82/84/88/92/95/98/101/104/";
