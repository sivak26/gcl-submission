...
/**
 * Class to accept the application details bean and submit DV Lottery application(s)
 *
 * @author sandeep
 *
 */
public class PostApplication {
     ...
     *
     * @param thisApp The bean which holds all the details necessary to submit DV application(s)
     */
    ...
    /**
     * Attempt submitting a single application. If the parameter appType is "P" attempts a submission for primary
     * applicant. If "S" then attempts a submission for spouse by swapping the applicant information with spouse
     *
    ...
    /**
     * Method to cancel the application submit process. This method would be called if any errors are encountered while
     * submitting. Errors may be because of insufficient data populated etc. This is a two step process, which involves
     * posting a cancel request with the session valules "__VIEWSTATE" and "__EVENTVALIDATION" passed, receive a
     * "Confirm cancel" Page and again post a "Confirm Cancel" request and verify the cancellation was successful.
     *
     * @param url The url where Cancel post has to be done.
     * @param where A pass through parameter for debugging information in logs, passed on the postData method which
     * makes use if it. where = which place/state is the code being executed
     * @return -1 if posting failed, http response code from host server after the post otherwise
     */
    ...
    /**
     * Update SubmitStatus table with confirmation number from the response received against submit application post
     *
     * @param userId UserId of the application submitted
     * @param response Response received against submit application post
     * @param appType P -> Primary applicant, S -> Spouse. Applicable when Couple Plan is opted
    ...
    /**
     * Parses the response received as a byte array to look for confirmation number and returns the same
     *
     * @param response Response to submit application request as a byte[] array
     * @return Empty string if Confirmation number not found, Confirmation number otherwise
     */
